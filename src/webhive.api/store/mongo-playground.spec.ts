...
describe('mongodb playground', () => {
    it.skip('create entry', async () => {
        const client = await new MongoClient(config.get('mongoUri'), {
            useUnifiedTopology: true,
        }).connect();
        const db = client.db(config.get('mongoDb'));
        const entryCollection = db.collection('entry2');
        const { result, insertedId } = await entryCollection.insertOne({
            a: new Date(),
            b: new Date().toString(),
        });
        client.close();
    ...
    it.skip('EntryService.create', async () => {
        ...
        const {
            ...
            title: 'A2 title',
            link:
                'http://airmanship.com/kansa/carapacho?a=stypticity&b=prediminution#pumpkinification',
    ...
    it.skip('lookup category', async () => {
        ...
        const result = await collection.aggregate([{ $lookup }]).limit(1).toArray();
